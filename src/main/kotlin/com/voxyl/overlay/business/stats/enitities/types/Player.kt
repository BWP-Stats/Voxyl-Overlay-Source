...
class Player(
    ...
    init {
        try {
            ...
            data["bwp.main"] = calcBWPMainMode(gameStatsJson?.json)
        } catch (e: Exception) {
    ...
    private fun calcBWPMainMode(json: JsonObject?): String {
        val stats = json["stats"] ?: return "ERR"

        val generalKeys = BwpMainMode.KEYS_MAP.keys

        val winsByMode = stats.entrySet()
            .associate { entry ->
        ...
        val sumByMode = groupByMode.mapValues { (_, keys) ->
            keys.sumOf { winsByMode[it]!! }
        }

        return sumByMode.maxByOrNull { it.value }?.key ?: "ERR"
    }
}
